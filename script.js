...
const ROTATION_PADDING = 1.2; // Espace supplémentaire pour la rotation
const MAX_CANVAS_SIZE = 550; // Taille maximale du canvas pour s'adapter aux écrans mobiles
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            originalImage.onload = function() {
                ...
                // Calculer le facteur de mise à l'échelle pour s'adapter à la taille max
                if (imgW > MAX_CANVAS_SIZE || imgH > MAX_CANVAS_SIZE) {
                    scaleFactor = MAX_CANVAS_SIZE / Math.max(imgW, imgH);
                }
                
                // Dimensions de l'image ajustée
                const scaledW = imgW * scaleFactor;
                const scaledH = imgH * scaleFactor;

                // --- DÉFINITION DE LA TAILLE DU CANVAS ---
                // Le Canvas est dimensionné pour accueillir l'image ajustée + un padding pour la rotation
...
function drawScene() {
    ...
    // Redessiner l'image en utilisant les dimensions originales et en appliquant le facteur d'échelle calculé
    let imgW = originalImage.width;
    let imgH = originalImage.height;
    let scaleFactor = 1;
